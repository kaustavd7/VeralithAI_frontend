...
export function LoadingState({ label = 'Loading…' }: { label?: string }) {
  return (
    <div style={wrap} role="status" aria-live="polite">
      ...
      <style>{`@keyframes sv-spin{to{transform:rotate(360deg)}}
        .sv-spinner{width:22px;height:22px;border-radius:50%;
          border:2px solid color-mix(in oklab, var(--po-fg) 14%, transparent);
          border-top-color:var(--accent);animation:sv-spin .7s linear infinite}`}</style>
    </div>
  );
}
