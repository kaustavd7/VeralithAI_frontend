...
`context` accepts a `list[str]`, a `list[dict]` (each with a `text` field), or a `list` of `ContextChunk` — so it fits whatever shape your chunks are already in. See [Context formats](/integration/context-formats).
